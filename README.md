...
| 8 | `TypeError: ... convert Float8_e4m3fn ...` from a **custom node's own FP8 Linear** (e.g. WanVideoWrapper `custom_linear.py`) | These bypass `comfy.ops` and cast FP8 weights/bias at runtime with a plain Python `.to(input)`; MPS can't cast to/from FP8 | Wrap `torch.Tensor.to` so FP8↔float conversions on MPS go through the LUT decode (FP8→float) or CPU (float→FP8); everything else takes a tight fast path. `.float()` / `.half()` / `.bfloat16()` bind to their own aten ops rather than routing through `.to()`, so they are wrapped as well — otherwise an fp8 tensor reaching one of them still raises (issue [#16](https://github.com/pawel-mazurkiewicz/ComfyUI-AppleSilicon-FP8/issues/16): W4A8-mixed checkpoints store fp8 group scales, and comfy_kitchen's int4→int8 dequant calls `s_rel.float()` on them) |
...
[AppleSilicon-FP8/tensor_to] torch.Tensor.to (+ .float()/.half()/.bfloat16()) FP8<->float routed via LUT/CPU on MPS.
